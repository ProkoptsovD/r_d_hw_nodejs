...
export class ChatService {
	...
	static async createUser(name: string, file?: File): Promise<UserDTO> {
		...
		return await ChatService.request<UserDTO>('admin', '/api/users', {
			method: 'POST',
			body: fd,
		});
	}
	...
	async listUsers(): Promise<UserDTO[]> {
		const res = await this.request<{ items: UserDTO[] }>(`/api/users`);
		return res.items;
	}

	/** GET /api/chats/:id/messages?cursor&limit=50 */
	async listMessages(chatId: string, cursor?: string): Promise<MessageDTO[]> {
		const res = await this.request<{ items: MessageDTO[] }>(`/api/chats/${chatId}/messages`, { params: { cursor, limit: 50 } });
		return res.items;
	}

	/** GET /api/chats/:id/members */
	async updateMembers(chatId: string, add: string[] = [], remove: string[] = []) {
		await this.request<void>(`/api/chats/${chatId}/members`, {
			method: 'PATCH',
			body: { add, remove },
		});
	}
	...
	async sendMessage(chatId: string, text: string): Promise<void> {
		await this.request<void>(`/api/chats/${chatId}/messages`, { method: 'POST', body: { text } });
	}

	/** POST /api/chats { members, name? } */
	async createChat(members: string[], name?: string): Promise<ChatDTO> {
		return this.request<ChatDTO>('/api/chats', { method: 'POST', body: { members, name } });
	}
	...
	static async request<T>(
		...
		const init: RequestInit = {
			...
			headers,
			signal: opts.signal,
			body: isFD ? (opts.body as FormData) : opts.body ? JSON.stringify(opts.body) : undefined,
		};
