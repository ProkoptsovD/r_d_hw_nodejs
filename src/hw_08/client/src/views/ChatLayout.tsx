...
import { Box, Divider, IconButton, List, Menu, MenuItem, Typography } from '@mui/material';
...
export const ChatLayout = ({ userName }: { userName: string }) => {
	...
	/* ---------------- chat + members events -------------- */
	useEffect(() => {
		const c1 = svc.onChatCreated().subscribe((c) => setChats((p) => [...p, c]));
		const c2 = svc.onMembersUpdated().subscribe((u) =>
			setChats((p) => {
				const result = p.map((c) => (c.id === u.chatId ? { ...c, members: u.members } : c)).filter((c) => c.members.includes(userName));
	...
	const onSend = useCallback((txt: string) => currentIdRef.current && svc.wsSend(currentIdRef.current, txt), [svc]);

	const onTyping = useCallback((flag: boolean) => currentIdRef.current && svc.wsTyping(currentIdRef.current, flag), [svc]);
		...
		<Box display="flex" height="100vh">
			...
			<Box width={280} borderRight="1px solid #ddd" display="flex" flexDirection="column">
				...
				<List sx={{ flexGrow: 1, overflowY: 'auto' }}>
					{chats.toReversed().map((c) => (
						<ChatItem key={c.id} chat={c} selected={c.id === current?.id} onSelect={selectChat} you={userName} />
					))}
				</List>
			</Box>
			...
			<Box flexGrow={1} display="flex" flexDirection="column">
				{current ? (
					<>
						{/* header */}
						<Box p={2} borderBottom="1px solid #ddd" display="flex" alignItems="center" justifyContent="space-between">
						...
						{/* input */}
						<MessageInput disabled={!current} onSend={onSend} onTyping={onTyping} />
					</>
				) : (
					<Box m="auto">
